...
<script>
    document.addEventListener("DOMContentLoaded", function() {
        document.querySelectorAll('.limit-btn').forEach(btn => {
            btn.addEventListener('click', function() {
                this.closest('.btn-group').querySelectorAll('.limit-btn')
    ...
    document.addEventListener("DOMContentLoaded", function() {
        ...
        // limit buttons inside a container
        function applyLimitButtons(container) {
            container.querySelectorAll('.limit-btn').forEach(btn => {
                btn.addEventListener('click', function() {
                    container.querySelectorAll('.limit-btn').forEach(b => b.classList.remove(
                        'active'));
                    this.classList.add('active');
        ...
        function applyLimit(container, limit) {
            ...
            const
                infoEl = container.querySelector('.showing-info');
            if (infoEl) infoEl.innerText = `Showing 1 to ${shown} of
    ${allRows.length} entries`;
        } // ── Init: apply default limit of 5 on load/open ─────────────────────── function
        initTable(container) {
        ...
        // Init all on-page cards on load
        document.querySelectorAll('.card').forEach(card => initTable(card));
