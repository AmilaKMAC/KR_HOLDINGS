                    ...
                    @forelse($assignedProjects as $assign)
                        ...
                        <tr>
                            ...
                            <td>
                                ...
                                @elseif ($proof && $proof->unapprove_reason)
                                    <span class="badge bg-danger">Not Approved</span><br>
                                    <button class="btn btn-sm btn-outline-danger mt-1"
                                            data-bs-toggle="modal"
                                            data-bs-target="#reasonModal{{ $assign->Project_idProject }}">
                                        View Reason
                                    </button>
                                @elseif ($proof)
...
{{-- ===== MODALS (one per project) ===== --}}
@foreach ($assignedProjects as $assign)
    @php
        $proof        = $proofRecords[$assign->Project_idProject] ?? null;
        $isUnapproved = $proof && $proof->unapprove_reason && $proof->approval == 0;
        $isApproved   = $proof?->approval == 1;

        // For unapproved: all sections re-uploadable
        // For normal: skip sections already uploaded
        $uploadedSections = ($proof && !$isUnapproved)
            ? $proof->images->where('is_reuploaded', 0)->pluck('section')->unique()->toArray()
            : [];

        $allUploaded = !$isUnapproved && count($uploadedSections) === count($sections);
    @endphp

    {{-- ===== REASON MODAL ===== --}}
    @if ($proof && $proof->unapprove_reason)
        <div class="modal fade" id="reasonModal{{ $assign->Project_idProject }}" tabindex="-1">
                ...
                <div class="modal-body">
                    ...
                    @if ($isUnapproved)
                        <div class="alert alert-danger">
                            ...
                            Coordinator reason: {{ $proof->unapprove_reason }}
                        </div>
                    @endif
                    ...
                    @if (!$isApproved && !$allUploaded)
                        <form method="POST" action="{{ route('proof_of_work.upload') }}"
                            ...
                            <div class="card-header bg-primary text-white fw-bold">
                                {{ $isUnapproved ? 'Re-upload Photos — Replace sections below' : 'Upload Photos — Select images for each section' }}
                            </div>
                            <div class="card-body">
                                ...
                                <div class="row g-3">
                                    @foreach ($sections as $key => $label)
                                        @if (!in_array($key, $uploadedSections))
                                            <div class="col-md-6">
                                                <div class="border rounded p-3 {{ $isUnapproved ? 'border-danger' : '' }}">
                                                    <label class="form-label fw-bold mb-2">
                                                        {{ $label }}
                                                        @if ($isUnapproved)
                                                            <span class="badge bg-danger ms-1">Re-upload</span>
                                                        @endif
                                                    </label>
                                                    <input type="file"
                                                           name="images[{{ $key }}][]"
                                ...
                                <div class="mt-4">
                                    <button type="submit" class="btn btn-primary px-5">
                                        {{ $isUnapproved ? 'Re-upload Photos' : 'Upload All Selected Photos' }}
                                    </button>
                                </div>
                            </div>
                        </form>
                    ...
                    {{-- Show uploaded photos (non-reuploaded only) --}}
                    @if ($proof && $proof->images->where('is_reuploaded', 0)->isNotEmpty())
                        <h6 class="fw-bold mb-3">
                            {{ $isUnapproved ? 'Previously Uploaded Photos' : 'Uploaded Photos' }}
                        </h6>
                        <div class="accordion" id="accordion{{ $assign->Project_idProject }}">
                            @foreach ($sections as $key => $label)
                                @php
                                    $sectionImages   = $proof->images->where('section', $key)->where('is_reuploaded', 0)->values();
                                    $alreadyUploaded = in_array($key, $uploadedSections);
                                @endphp
                                @if ($alreadyUploaded && $sectionImages->isNotEmpty())
